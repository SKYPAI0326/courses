...
| v1.3.3 | 2026-09-13 | 不再限制 Gemini key 前綴；支援新建 `AQ.` authorization key |
| v1.3.5 | 2026-09-13 | smoke test 與 LP helper 改用 `thinkingLevel: minimal`，相容 `gemini-3.6-flash`；失敗時保留 HTTP 狀態與完整回應 |
...
- 2026-09-13：便箋依目前 n8n 介面改為橘色 `Execute workflow`，補充 `message`、`timestamp`、`received_method` 三個輸出欄位；workflow 邏輯與版本維持不變。
- 2026-09-13：便箋測試步驟改以執行／等待、Test URL 回應與綠框 Output 為主；藍色 `Listening for test event` 改列為版本差異提示。
- 2026-09-13：#04 修正來源檔名回查、來源清單與衝突／未指定欄位規則，並在輸出 JSON 提供 `filesCount`、`filesProcessed`、`sourceFiles`；AI 回應缺少任一來源標記時改為人工核對狀態。
...
- v1.2：LP helper
- v1.3：$env.GEMINI_API_KEY
- 2026-09-13：更新目前 n8n 等待狀態文案、標明 5 節點 POST 主路徑與 3 節點 Web UI 分支，並統一 C6 驗收條件。
- 2026-09-13：將 #06 Webhook responseMode 改為 `responseNode`，由 Respond to Webhook 固定回傳 `status/question/answer`，避免 fan-out 時誤回 Telegram API JSON。
