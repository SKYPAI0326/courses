# n8n Lite Pack · setup-wizard.ps1 (Windows)
...
"$(Get-Date) - setup-wizard 開始" | Out-File $Log -Encoding UTF8

Write-Host ""
Write-Host "═════════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host "  n8n Lite Pack · setup-wizard for Windows v0.9" -ForegroundColor Cyan
...
try {
  ...
  Write-Host "  請打開 Docker Desktop（工具列鯨魚 icon）等待啟動完成後再次執行" -ForegroundColor Red
  exit 1
}
...
try {
  ...
  Write-Host "  ✓ Python 3: $pyVersion" -ForegroundColor Green
  $PythonCmd = "python"
} catch {
  try {
    $pyVersion = python3 --version 2>&1
    if ($LASTEXITCODE -ne 0) { throw "no python3" }
    Write-Host "  ✓ Python 3: $pyVersion" -ForegroundColor Green
    $PythonCmd = "python3"
  } catch {
    Write-Host "  ❌ 找不到 Python 3" -ForegroundColor Red
    Write-Host "  請到 Microsoft Store 搜尋 'Python 3' 安裝（免費），完成後重跑本檔" -ForegroundColor Red
    exit 1
  }
}
...
# 1e. n8n container
$N8nContainer = (docker ps --filter "name=n8n" --format "{{.Names}}" | Select-String -Pattern "n8n-\d+$|n8n_1" | Select-Object -First 1).Line
if (-not $N8nContainer) {
  $N8nContainer = (docker ps --filter "name=n8n" --format "{{.Names}}" | Select-Object -First 1)
}
if (-not $N8nContainer) {
  Write-Host "  ❌ 找不到 n8n container" -ForegroundColor Red
  exit 1
}
...
if (-not (Test-Path "$StarterKit\n8n-compose.yml")) {
  $StarterKit = Read-Host "找不到預設位置的 n8n-starter-kit。請貼上完整路徑（含 n8n-compose.yml 的資料夾）"
  if (-not (Test-Path "$StarterKit\n8n-compose.yml")) {
...
Write-Host "  ✓ workflows 資料夾就緒" -ForegroundColor Green

# ════════ Step 2: 收集學員資料 ════════
Write-Host ""
Write-Host "[2/10] 收集 personalization 資料..." -ForegroundColor Yellow

$GeminiKey = Read-Host "請貼上你的 Gemini API key（AIza... 開頭。沒有？到 https://aistudio.google.com/apikey 申請）"
if ($GeminiKey -notmatch '^AIza') {
  Write-Host "  ❌ Gemini API key 格式不對（應以 AIza 開頭）" -ForegroundColor Red
  exit 1
}

$TgToken = Read-Host "請貼上你的 Telegram bot token（格式如 123456:ABC-DEF...。沒有？Telegram 找 @BotFather → /newbot）"
if ($TgToken -notmatch '^\d+:') {
  Write-Host "  ❌ Telegram bot token 格式不對" -ForegroundColor Red
  exit 1
}

$TgChatId = Read-Host "請貼上你的 Telegram Chat ID（純數字。對 bot 發訊息後開 https://api.telegram.org/bot<TOKEN>/getUpdates 找 chat.id）"
...
if ($ComposeContent -match 'N8N_RESTRICT_FILE_ACCESS_TO') {
  ...
  $patched = $ComposeContent -replace 'N8N_BASIC_AUTH_ACTIVE=false', "N8N_BASIC_AUTH_ACTIVE=false`n      - N8N_RESTRICT_FILE_ACCESS_TO=/files/shared`n      - N8N_BLOCK_FILE_ACCESS_TO_N8N_FILES=false"
...
if ($RestartNeeded) {
  ...
  docker compose -f n8n-compose.yml down 2>&1 | Out-Null
  docker compose -f n8n-compose.yml up -d 2>&1 | Out-Null
  Pop-Location
...
$Folders = @('pdf-inbox','pdf-renamed','batch-inbox','batch-inbox\processed','batch-inbox\failed','daily-input','daily-output','ai-output','client-inbox','client-organized','leads-inbox','leads-output','knowledge-docs','knowledge-index','ops-input','ops-history','ops-snapshots','ops-incidents')
foreach ($sub in $Folders) {
  New-Item -Path "$Shared\$sub" -ItemType Directory -Force | Out-Null
}
Write-Host "  ✓ shared/ 子資料夾建好（位於 $Shared）" -ForegroundColor Green

# ════════ Step 6: 生成 credentials JSON ════════
Write-Host ""
...
# ════════ Step 7: 替換 workflow placeholder ════════
Write-Host ""
...
Get-ChildItem "$WorkflowTmp\*.json" | ForEach-Object {
  ...
  $content = $content.Replace('__TELEGRAM_CHAT_ID__', $TgChatId).Replace('__GEMINI_API_KEY__', $GeminiKey)
...
docker cp $CredFile "${N8nContainer}:/tmp/credentials.json" | Out-Null
$credResult = docker exec -u node $N8nContainer n8n import:credentials --input=/tmp/credentials.json 2>&1
$credResult | Out-File -Append $Log -Encoding UTF8
if ($LASTEXITCODE -ne 0) {
  Write-Host "  ❌ credentials 匯入失敗" -ForegroundColor Red
  Write-Host $credResult -ForegroundColor Red
  Write-Host "  常見原因：n8n 沒建 Owner Account。先到 http://localhost:5678 完成 owner setup" -ForegroundColor Yellow
  exit 1
}
Write-Host "  ✓ $credResult" -ForegroundColor Green

# ════════ Step 9: import workflows ════════
Write-Host ""
Write-Host "[9/10] 匯入 14 個 workflows 到 n8n..." -ForegroundColor Yellow

Get-ChildItem "$WorkflowTmp\*.json" | Sort-Object Name | ForEach-Object {
  $f = $_.Name
  docker cp $_.FullName "${N8nContainer}:/tmp/$f" | Out-Null
  $wfResult = docker exec -u node $N8nContainer n8n import:workflow --input=/tmp/$f 2>&1
  $wfResult | Out-File -Append $Log -Encoding UTF8
  if ($LASTEXITCODE -eq 0) {
    Write-Host "  ✓ $f" -ForegroundColor Green
  } else {
    Write-Host "  ⚠ $f 匯入失敗（看 log）: $wfResult" -ForegroundColor Yellow
  }
}
...
# Telegram
try {
  $tgResp = Invoke-RestMethod -Uri "https://api.telegram.org/bot$TgToken/sendMessage" -Method Post -ContentType "application/json" -Body (@{chat_id=$TgChatId; text="✅ Lite Pack v0.9 setup-wizard 安裝完成！(Windows)"} | ConvertTo-Json) -TimeoutSec 10
  if ($tgResp.ok) {
...
try {
  ...
  $geminiResp = Invoke-RestMethod -Uri "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent" -Method Post -Headers @{"x-goog-api-key"=$GeminiKey} -ContentType "application/json" -Body $geminiBody -TimeoutSec 30
...
Write-Host "  2. 左側 Workflows 應該看到 14 個（編號 01 ~ 14）"
