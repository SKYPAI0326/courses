...
def validate_pack():
    """v1.1：生成後逐檔驗證（可讀性 + 副檔名 magic 對齊）。Codex 4cb89bc7 採納。"""
    issues = []
    file_count = 0
    for path in sorted(OUT.rglob("*")):
        ...
        try:
            ...
            # 0 byte 檢查（empty.txt 是預期，其他不該）
            if size == 0 and path.name != "empty.txt":
            ...
            if ext == ".pdf":
                with open(path, "rb") as f:
                    head = f.read(5)
                if head != b"%PDF-" and path.name != "not-a-pdf.pdf":
