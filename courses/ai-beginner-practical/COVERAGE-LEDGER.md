...
| CH2-A05 | core | OP-03 / 單一變因迭代 | 至少兩份成品各只改一個主要條件 | 工作台匯出的前後版本與修改理由 | 寫出改了什麼、保留什麼；刪掉關鍵事實回 U2-ITERATE | CH2-1 `#communication-workbench`、`U2-ITERATE`、`U2-SAVE` |
| CH3-A01 | core | OP-04 / 來源建立 | 先在紀錄台留下筆記本與來源，再建立主題筆記本並加入 3 份課堂來源 | 含來源的 NotebookLM 筆記本＋紀錄台狀態 | 來源列表有 3 個可辨認名稱；失敗回 U3-SOURCE 並標待重跑 | CH3-1 `#notebooklm-log`、`U3-START`、`U3-SOURCE` |
| CH3-A02 | core | OP-05 / 摘要層次 | 對同一篇新聞提出一句話、約 200 字、約 300–350 字 | 三段式新聞摘要 | 主旨一致、286 與 174 保留；太短回 U3-SUMMARY | CH3-1 `U3-SUMMARY` |
| CH3-A03 | core | OP-04 / 引用回查 | 點開至少一個正式引用，回到原文核對數字或限制，再填回紀錄台 | 引用核對紀錄 | 引用能回到原文；對不上拆短句回 U3-CITE | CH3-1 `#notebooklm-log`、`U3-CITE`、`U3-SAVE` |
...
| CH4-A01 | core | OP-06 / 條件進入結果 | 在 CH4-1 工作台從主卡填目標、時間、預算、偏好、限制 | 工作台中的個人條件清單 | 目標可看見、限制至少 1 項；缺欄回 U4-SELECT | CH4-1 `#lifestyle-workbench`、`U4-SELECT` |
...
| CH4-A05 | core | OP-06 / 可重做保存 | 從工作台匯出原始需求、提示詞、兩版與檢核 | `unit4-lifestyle-application-card.md` | 隨機開啟仍找得到兩版與待確認事項；缺段回 U4-SAVE-2 | CH4-1 `#lifestyle-workbench`、`U4-SAVE-2` |
...
| CH2-1 | 對不同讀者寫日常文字的成人 | 讀者／目的／語氣不清造成誤解與重寫 | 頁面日常溝通工作台、`assets/templates/unit2-communication-scenarios.md` 或 HTML 素材頁；工具不可用標待重跑，Markdown 作離線備援 | 工作台匯出的 `unit2-communication-pack-complete.md` | 自己、實際收件者；CH4 個人化 | 開啟工作台填第一個情境；看到三類成品區與檢核表 | `#communication-workbench`、`U2-START`、`U2-PROMPT`、`U2-EMAIL`／`U2-MESSAGE`、`U2-ITERATE` |
| CH3-1 | 需要回查長文的成人 | 把補寫內容當原文會誤判日期／條件／下一步 | 頁面 NotebookLM 閱讀紀錄台、NotebookLM、來源索引與三份必做來源；平台不可用標待重跑並用純文字 fallback | 紀錄台匯出的 `unit3-notebooklm-reading-pack-complete.md` + NotebookLM 筆記本 | 自己或工作夥伴回查；CH4 轉用 | 先填紀錄台，再建立筆記本並加入三份來源；兩處都能觀察來源狀態 | `#notebooklm-log`、`U3-SOURCE`、`U3-SUMMARY`、`U3-ANNOUNCE`、`U3-CITE` |
| CH4-1 | 有近期生活任務的成人 | 泛問會猜條件，造成不合用或越界結果 | 頁面生活應用工作台、30 張提示詞卡、應用卡 Markdown 備援、冷氣資料與文字型 LLM；工具不可用先填欄位並標待重跑 | 工作台匯出的 `unit4-lifestyle-application-card.md` | 未來自己重做；課後交 capstone handoff | 開啟工作台、選主卡、填五欄；看到主卡與限制 | `#lifestyle-workbench`、`U4-SELECT`、`U4-PROMPT`、`U4-CHECK-1`、`U4-REVISE`、`U4-SAVE-2` |
