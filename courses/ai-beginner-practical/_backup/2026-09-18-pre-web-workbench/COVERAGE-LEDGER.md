...
| CH1-A03 | core | worked operation | E-07,E-08,E-09,E-10 | OP-01 / 自我介紹提問 | CH1-1.html#lesson-example-bank 案例 1；#lesson-practice 步驟 2 | 案例 1 同時給出完整輸入、可用輸出、過度承諾錯例與修正指令；步驟 2 要學員對照後實跑 | `unit1-practice-sheet.md` 第 1 題 | 先讀案例 1，再送出提示詞、保存完整回答並圈出三類能力與確認提醒 | 紀錄表出現自我介紹回答、觀察與可重做修正 | 偏題或過度承諾時追加「列出三類文字協助並標示需要確認的內容」再跑一次 | CH1 |
| CH1-A04 | core | worked operation | E-11,E-12,E-13 | OP-02 / 即時問題的限制與改問 | CH1-1.html#lesson-example-bank 案例 2；#lesson-practice 步驟 3 | 案例 2 示範無即時資料時的完整停損輸出，並對照沒有時間與來源的晴雨錯例 | `unit1-practice-sheet.md` 第 2 題 | 先讀案例 2，再送出天氣問題並記錄日期、時間、來源或無法判斷的原因 | 紀錄表保留回答依據與下一步查證方向 | 缺少時間或來源時追加「先說明資料時間與來源；無法取得時不要自行填寫數字」 | CH1 |
| CH1-A05 | core | worked operation | E-14,E-15,E-16 | OP-02 / 晚餐建議 | CH1-1.html#lesson-demo；#lesson-practice 步驟 4 | 完整示範提供模糊輸入、五欄條件、三列輸出表與四條件核對 | `unit1-practice-sheet.md` 第 3 題 | 送出晚餐提示詞並圈出兩人、不吃牛、600 元、30 分鐘四個條件 | 表格含三個選項、時間、預算與仍缺條件的說明 | 忽略條件時只補被忽略的條件再跑一次，保留原版作比較 | CH1 |
| CH1-A06 | core | worked operation | E-17,E-18,E-19 | OP-01 / ETF 白話解釋 | CH1-1.html#lesson-example-bank 案例 4；#lesson-practice 步驟 5 | 案例 4 給出一句定義、生活例子、三個待查問題，以及把解釋誤寫成推薦的錯例 | `unit1-practice-sheet.md` 第 4 題 | 先讀案例 4，再送出提示詞、用自己的話重述 ETF 並保存待查問題 | 重述包含一籃子資產與市場交易兩個意思，且沒有商品推薦 | 太專業或開始推薦時追加「只做概念解釋，不推薦商品」 | CH1 |
| CH1-A07 | core | worked operation | E-20,E-21,E-22 | OP-01 / 通知整理 | CH1-1.html#lesson-example-bank 案例 5；#lesson-practice 步驟 6 | 案例 5 給出原文、三個完整條列、五項資訊核對與漏掉時間的錯例 | `unit1-practice-sheet.md` 第 5 題 | 先讀案例 5，再整理通知成三個條列並逐項標記五項資訊 | 三個條列保留日期、所有時間、住戶行動與雨天備案，未加入新規定 | 漏掉重要時間時追加「先列出五項資訊，再整理成三個條列」並重跑 | CH1 |
...
| CH1-A09 | core | finished artifact | E-26,E-27,E-28 | OP-01,OP-02 / 保存實務紀錄 | CH1-1.html#lesson-practice 步驟 9；#lesson-check | 保存步驟與完成驗證明定五題、前後版本、starter／個人三段紀錄、待查項目與檢查方式 | 填寫中的 `unit1-practice-sheet.md` | 保存為 `unit1-practice-sheet-complete.md` 並逐項檢查 | 沒參加課程的人能看懂原始問題、回答、個人轉移與待查處 | 缺原始提示詞時回到 U1-SAVE 補回，不刪除既有回答 | CH1 → CH2/CH4 |
...
|---|---|---|---|---|---|---|---|
| CH1-1 | 文字型 LLM 初學成人；工作／生活小任務 | 模糊提問造成泛答、漏條件，無法重做 | `assets/worksheets/unit1-practice-sheet.md`；LLM 不可用看 `assets/fallback/unit1-dialogue-simulator.md` | `unit1-practice-sheet-complete.md` | 自己在 CH2／CH4 重用五欄與版本 | 複製工作表副本並填姓名日期；看到五題欄位與貼回答位置 | `U1-START`、`U1-ASK`、`U1-REVISE`；保存副本後停止 |
