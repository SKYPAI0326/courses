...
## 2026-08-25 Content Alignment Repair Addendum

1. Prompt 密度檢查通過：保留講義中的完整 Prompt；投影片可依投影片規格取用結構、示範與全文入口。
2. CH2 品牌主線統一為「弄一下咖啡」；CH1-3 的「午後咖啡館」保留為獨立照片辨識變體，並在素材中明示不屬 CH2 品牌資料庫。
3. CH1-3 主線改用「手工皂創業者／手工皂工坊」；EF-04 延伸情境改用不具名角色標籤；資安 NG/OK 示範改用 `{真實姓名}`、`{電話}`、`{地址}` 等佔位符。
4. 「每週至少 2–3 小時」及流程分鐘數改為課程目標、課堂示意估算或附實際時間變動說明；一般省時語句改為工作流建議；不改課程單元時數。
5. 本次修復備份位於 `_backup/2026-08-25-pre-repair/`；可用 `_tools/restore-2026-08-25-pre-repair.sh` 還原本次新增範圍。
