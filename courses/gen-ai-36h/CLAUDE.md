...
立刻執行：
1. 用 `mempalace_diary_write` 記下今日完成的檔案、目前進行到哪一步、下一個待辦
2. 告知：「已記錄，下次說『繼續 36h 課程』即可恢復」

### 恢復（使用者說「繼續 36h 課程」或「繼續上次的工作」）

1. `mempalace_diary_read` 最近 3 筆
2. 讀 `index.html` 確認目前哪些 lesson-card 還是 `locked` 狀態
...
- [ ] 密碼關卡（password gate）：尚未加入。建立第一個步驟頁前需確認密碼，再用 `inject_gate.py` 注入到 index.html 與所有子頁
- [ ] 統一課程入口 `courses/index.html` 是否要加入此課程卡片
