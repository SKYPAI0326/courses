...
阿凱把 B4 的 `Screen / Host` 與 `Dialog / Overlay` 交給雯姊試用。雯姊說：「我看得到畫面，但不知道點擊後是不是應該出現這個訊息。」這不是審美問題，而是任務、操作與預期結果沒有被固定下來。本單元把「我覺得可以」改成一張可重跑的測試表。
...
阿凱要交付一份能讓雯姊審稿的測試紀錄。雯姊不是要看更多連線，而是要知道：從哪裡開始、做什麼、應看到什麼、如果沒看到要怎麼分類與回修。本單元的交付物是這份可回溯紀錄，不是口頭保證。
