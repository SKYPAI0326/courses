...
| Bottom bar 高度 | 52px |
| Sidebar 寬度 | 260px |
| 邊線樣式 | 1px solid hairline（無 hard drop shadow、無 gradient） |
| 圓角上限 | 8px（禁止超過） |

**Shadow 規則精準化（2026-05-12，audit 啟發自 tw93/Kami）**：

- ❌ 禁止 hard drop shadow：`box-shadow` 帶 `offset > 0` 或 `blur > 4px`（產生強烈飄浮感、與無印風衝突）
- ✅ 允許 ring（內陰影擬態 1px hairline border）與 whisper（極弱 1-2px blur、opacity ≤ 0.06 的環境陰影）
- 機器強制：既有 `check_box_shadow`（lint-page.py:109）的偵測邏輯涵蓋此規則、不需新增 check
...
### Line-height 建議區間（軟規範，2026-05-12 audit-driven）

> 啟發自 tw93/Kami 的三層 line-height 規格，但本機現有頁 audit 顯示 CJK 內文需更寬鬆行距（拉丁字距 1.5-1.55 對中文太緊），故**不直接抄 Kami 數值**、改為「audit-driven CJK 適配版」。
>
> 此規則為**軟規範（建議）**、`lint-page.py` **不 enforce**（避免噴 WARN 干擾正常 lint 訊號）。新建單元頁時參考此區間；既有頁不強制回溯修改。

| 層級 | 建議區間 | 典型用途 | 本機現有頁實證使用值 |
|------|---------|---------|---------------------|
| 標題（tight）| 1.1-1.4 | `.lesson-title` / `.page-title` / `.section-heading` | 1.2 / 1.3 / 1.35 / 1.4 |
| 密 body（卡片 / 短段）| 1.5-1.7 | `.tool-summary` / `.callout-body` / `.body-text` 短段 | 1.5 / 1.55 / 1.6 / 1.65 / 1.7 |
| 閱讀 body（長段）| 1.75-1.95 | SECTION 01 破題段、reflection-block、長 `.body-text` | 1.75 / 1.8 / 1.85 / 1.9 / 1.95 |

**例外（不適用此規則）**：
- px-based 元素（icon / badge / `width=height=line-height` 垂直居中技巧）→ 用 px 單位、不受此區間限制
- `line-height: 1` → 用於需要嚴格貼齊基線的場景（如純圖示對齊）

**為什麼 CJK 需要更寬鬆行距**：中文字符方塊大小一致、無上升 / 下降部（ascender / descender），緊行距會讓筆畫密集視覺擁擠；拉丁字 (Charter / Inter) 有自然垂直空隙、可用 1.5-1.55。差距約 +0.3。
